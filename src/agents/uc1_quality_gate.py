...
## 임무
추출된 뉴스 기사가 수집 기준에 적합한지 **합리적으로** 검증하세요.
...
## 소스 신뢰 (Source Trust)
- URL에 `/economy/`, `/politics/` 등 카테고리 경로가 포함되어 있으면, 해당 소스가 이미 분류한 것이므로 **카테고리 신뢰도 높음**
- 예: `/economy/all`에서 가져온 기사는 경제 관련으로 가정

## 검증 기준 (모두 통과해야 함!)

### 1. 카테고리 적합성 (Category Match)
**기준**: 기사 내용이 목표 카테고리와 **합리적으로** 일치하는가?

**카테고리별 예시**:

**경제 (economy)**:
✓ 삼성전자 실적, 코스피 지수, 금리 정책, 부동산 시장, 환율, 기업 인수합병
✓ 기업 인사/경영진 변화 (비즈니스 영향), 팝업스토어/신제품 출시 (경제 활동)
✓ 연예인 모델 계약/광고 산업, 기업 복지/채용 (경제 활동)
✗ 순수 연예 뉴스 (BTS 신곡 발매), 스포츠 경기 결과, 정치인 개인 발언 (경제 정책 제외)

**정치 (politics)**:
✓ 국회 법안, 대통령 발언, 정당 활동, 선거, 정부 정책
✓ 공무원 인사 (정책 영향), 안보/외교 (정치 관련)
...
- URL에 카테고리 경로가 있으면 소스 분류를 신뢰
- 카테고리가 명확히 다르면 **uncertain** (UC2로 전달)
...
## Decision 규칙
- **pass**: 모든 기준 통과 + confidence >= 90
- **reject**: 명백한 광고/보도자료/홍보성, 또는 본문 심각하게 불완전
- **uncertain**: 애매한 경우 (70 <= confidence < 90) 또는 카테고리 불명확 → UC2 발동

## 중요!
- **합리적으로** 평가하세요. 의심스러우면 **uncertain** (UC2가 검증).
- 광고/보도자료/홍보성은 **reject** (단, 비즈니스 뉴스는 허용).
- 카테고리가 애매하면 **uncertain** (UC2로 전달).
- 소스 URL에 카테고리 경로가 있으면 신뢰도 높임.
- 본문이 500자 미만이면 내용 확인 후 판단 (짧은 속보는 200자 허용).
